  ...
  async function handleSubscribe(plan) {
    ...
    try {
      const res = await fetch("/api/billing/create-checkout", {
        ...
        body: JSON.stringify({ shop, planId, period }),
      });
      const data = await res.json();
      if (data.url) {
        window.location.href = data.url;
      } else {
        setToast({ msg: data.error || "Failed to create checkout.", type: "error" });
      }
    } catch {
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: T.bgBase, overflow: "hidden" }}>
      <style>{`
        ...
        .sub-btn { width:100%; border-radius:9px; padding:12px; font-size:14px; font-weight:700; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:7px; transition:all 0.2s; font-family:'DM Sans',sans-serif; border:none; }
        .sub-btn:hover:not(:disabled) { filter:brightness(1.08); transform:translateY(-1px); }
        .sub-btn:disabled { opacity:0.7; cursor:default; }
        @media (max-width:768px) { .hdr { padding:10px 16px 10px 52px !important; } .main-pad { padding:16px !important; } .plans-grid { grid-template-columns:1fr !important; } .faq-grid { grid-template-columns:1fr !important; } .hide-sm { display:none !important; } }
      `}</style>
      ...
      {toast && (
        <div style={{ position:"fixed", bottom:20, right:20, zIndex:1000, background:T.bgCard, border:`1px solid ${toast.type==="success"?T.greenBorder:T.orangeBorder}`, borderRadius:10, padding:"13px 16px", maxWidth:340, boxShadow:T.shadowMd, display:"flex", alignItems:"center", gap:10, animation:"toastIn 0.25s ease" }}>
          ...
          <button onClick={() => setToast(null)} style={{ background:"none", border:"none", cursor:"pointer", color:T.textTertiary, fontSize:18, marginLeft:"auto" }}>×</button>
        </div>
      )}
      ...
      <main style={{ flex:1, overflow:"auto" }}>
        <header className="hdr" style={{ padding:"12px 24px", background:T.bgCard, borderBottom:`1px solid ${T.borderSubtle}`, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:10, boxShadow:T.shadow }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8, background:T.greenSubtle, border:`1px solid ${T.greenBorder}`, borderRadius:8, padding:"6px 14px" }}>
            ...
            <span style={{ fontSize:12, fontWeight:700, color:T.green }}>{planStatus==="active" ? `${currentPlan} Plan Active` : "14-Day Free Trial"}</span>
          </div>
        </header>
        ...
        <div className="main-pad" style={{ padding:"28px" }}>
          ...
          <div style={{ textAlign:"center", marginBottom:36 }}>
            ...
            <p style={{ fontSize:14, color:T.textSecondary, maxWidth:480, margin:"0 auto 20px", lineHeight:1.6 }}>All plans include a 14-day free trial. No credit card charged until your trial ends.</p>
            {/* Period toggle */}
          ...
          {/* Plans */}
          <div className="plans-grid" style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:18, maxWidth:920, margin:"0 auto 28px" }}>
            {PLANS.map(plan => {
              ...
              return (
                <div key={plan.id} className="plan-card" style={{ background:plan.highlight?T.olive:T.bgCard, border:`2px solid ${isActive?T.greenBorder:plan.highlight?T.olive:T.borderSubtle}`, borderRadius:16, padding:"28px 24px", position:"relative", boxShadow:plan.highlight?`0 8px 32px rgba(61,90,62,0.2)`:T.shadow }}>
                  ...
                  <button className="sub-btn" onClick={() => handleSubscribe(plan)} disabled={loading===plan.id||isActive}
                    ...
                    {loading===plan.id
                      ? <><span style={{ width:14, height:14, border:"2px solid rgba(255,255,255,0.3)", borderTopColor:"white", borderRadius:"50%", display:"inline-block", animation:"spin 0.8s linear infinite" }}></span>Redirecting to Stripe...</>
                      : isActive ? "Current Plan ✓"
                      : plan.monthlyId ? "Subscribe with Stripe →" : "Contact Us →"}
                  </button>
                </div>
              );
            })}
          ...
          {/* Stripe badge */}
          <div style={{ maxWidth:920, margin:"0 auto 28px", background:T.bgCard, border:`1px solid ${T.borderSubtle}`, borderRadius:12, padding:"14px 20px", display:"flex", alignItems:"center", gap:12 }}>
            <div style={{ width:36, height:36, borderRadius:8, background:T.greenSubtle, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={T.green} strokeWidth="2"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>
            </div>
            <div>
              <div style={{ fontSize:13, fontWeight:700, color:T.textPrimary, marginBottom:2 }}>Secure payments powered by Stripe</div>
              <div style={{ fontSize:12, color:T.textSecondary }}>Your payment info is encrypted and never stored on our servers. Cancel anytime.</div>
            </div>
          </div>
          ...
          <div style={{ maxWidth:920, margin:"0 auto", background:T.bgCard, border:`1px solid ${T.borderSubtle}`, borderRadius:14, padding:"24px 28px", boxShadow:T.shadow }}>
            ...
            <div className="faq-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
              {[
                { q:"How does the free trial work?", a:"All plans include a 14-day free trial. Your card won't be charged until the trial ends. Cancel anytime before then." },
                ...
                { q:"What cards are accepted?", a:"Visa, Mastercard, American Express, and most major debit cards via Stripe's secure checkout." },
