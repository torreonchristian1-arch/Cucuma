...
export default function Dashboard() {
  ...
  if (!shop && !loading) return (
    <div style={{ minHeight: "100vh", background: theme.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20, padding: 24, textAlign: "center" }}>
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: theme.bg, overflow: "hidden", transition: "background 0.25s" }}>
      <style>{`
        ...
        .mcard { transition: transform 0.18s, box-shadow 0.18s; cursor: default; }
        .mcard:hover { transform: translateY(-2px); box-shadow: ${theme.shadowMd}; }
        .orow { transition: background 0.12s; cursor: pointer; }
        .orow:hover { background: ${theme.surfaceHover} !important; }
        .action-btn { transition: all 0.15s; cursor: pointer; }
        .action-btn:hover { transform: translateY(-1px); opacity: 0.88; }
      ...
      <main style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column" }}>
        ...
        <header className="hdr" style={{ padding: "14px 28px", background: theme.surface, borderBottom: `1px solid ${theme.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10, boxShadow: theme.shadow }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button className="action-btn hide-sm" onClick={() => goTo("/catalogue")}
              style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 9, padding: "8px 16px", color: theme.textSub, fontSize: 13, fontWeight: 600 }}>
              + Add Product
            </button>
            <button className="action-btn" onClick={() => goTo("/catalogue")}
              style={{ background: `linear-gradient(135deg, ${theme.gold}, #a07828)`, border: "none", borderRadius: 9, padding: "8px 18px", color: "white", fontSize: 13, fontWeight: 700, boxShadow: `0 4px 14px rgba(184,134,42,0.35)` }}>
              Publish to Store →
            </button>
          </div>
        </header>
        ...
        <div className="main-pad" style={{ padding: "24px 28px", flex: 1 }}>
          ...
          <div className="metrics-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 24 }}>
            {METRICS.map((m, i) => (
              <div key={i} className="mcard" style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 14, padding: "20px 22px", boxShadow: theme.shadow }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                  <div style={{ fontSize: 22 }}>{m.icon}</div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: m.up ? theme.green : theme.orange, background: m.up ? theme.greenBg : theme.orangeBg, border: `1px solid ${m.up ? theme.greenBorder : theme.orangeBorder}`, padding: "2px 8px", borderRadius: 100 }}>
                    {m.up ? "↑" : "↓"} {m.delta}
                  </div>
                </div>
                <div style={{ fontFamily: "'Fraunces', serif", fontSize: 26, fontWeight: 700, color: theme.text, lineHeight: 1, marginBottom: 6 }}>{m.value}</div>
          ...
          <div className="main-grid" style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 16 }}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 420 }}>
                  ...
                  <tbody>
                    {SAMPLE_ORDERS.map((o, i) => {
                      ...
                      return (
                        <tr key={i} className="orow" style={{ borderBottom: `1px solid ${theme.border}` }}>
                          ...
                          <td style={{ padding: "13px 18px" }}>
                            <span style={{ background: s.bg, color: s.color, border: `1px solid ${s.border}`, borderRadius: 100, padding: "3px 10px", fontSize: 11, fontWeight: 700 }}>{s.label}</span>
                          </td>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {/* Fulfillment */}
              <div style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 14, padding: 18, boxShadow: theme.shadow }}>
                <div style={{ fontFamily: "'Fraunces', serif", fontSize: 15, fontWeight: 700, color: theme.text, marginBottom: 14 }}>Fulfillment Pipeline</div>
                {[
                  { label: "Awaiting Pickup", count: 12, total: 54, color: "#f59e0b", bg: mode === "dark" ? "#1a1200" : "#fef9c3" },
                  { label: "In Transit", count: 24, total: 54, color: theme.blue, bg: theme.blueBg },
                  { label: "Delivered", count: 18, total: 54, color: theme.green, bg: theme.greenBg },
                ].map((f, i) => (
                  <div key={i} style={{ marginBottom: 14 }}>
                    ...
                    <div style={{ height: 6, background: theme.border, borderRadius: 100, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${(f.count / f.total) * 100}%`, background: f.color, borderRadius: 100, transition: "width 0.6s" }}></div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 14, padding: 18, boxShadow: theme.shadow }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                  <div style={{ fontFamily: "'Fraunces', serif", fontSize: 15, fontWeight: 700, color: theme.text }}>Webhooks</div>
                  <span style={{ fontSize: 10, fontWeight: 700, color: theme.green, background: theme.greenBg, border: `1px solid ${theme.greenBorder}`, borderRadius: 100, padding: "3px 10px", letterSpacing: "0.04em" }}>ALL ACTIVE</span>
                </div>
                {["orders/create", "orders/fulfilled", "orders/cancelled", "products/update"].map((w, i, arr) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: i < arr.length - 1 ? `1px solid ${theme.border}` : "none" }}>
                    <code style={{ fontSize: 11, color: theme.textSub }}>{w}</code>
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#16a34a" }}></div>
                      <span style={{ fontSize: 10, color: theme.green, fontWeight: 700 }}>Live</span>
                    </div>
                  </div>
                ))}
              </div>
